...
function Gamam150() {
  ...
  const totalSolved = Object.values(codingProblems).reduce((total, problems) => {
    return total + problems.filter(problem => localStorage.getItem(problem.name) === 'true').length;
  }, 0);
    ...
    <div className="gamam-tracker">
      ...
      <div className="day-list">
        {Object.entries(codingProblems).map(([day, problems]) => (
          <div key={day} className="problem-list">
            ...
            {problems.map((problem, problemNo) => (
              <div key={problem.name} className="problem-row">
